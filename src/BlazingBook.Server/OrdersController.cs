...
namespace BlazingBook.Server {
    ...
    public class OrdersController : Controller {
        ...
        public OrdersController(BookStoreContext db, IConfiguration config, ILogger<OrdersController> logger) {
            _db = db;
            _config = config;
            _logger = logger;
        }
        ...
        public async Task<ActionResult<List<OrderWithStatus>>> GetOrders() {
            ...
            _logger.LogInformation("List of books in the order {count}", orders.Count());
        ...
        public async Task<ActionResult<int>> PlaceOrder(Order order) {
            ...
            _logger.LogInformation("Order with id: {id} saved to Database at {time}", order.OrderId, DateTime.UtcNow);
            ...
            if (subscription != null) {
                ...
                _logger.LogCritical("Subscription null, I will send an email anyway", subscription);
        ...
        private async Task ExecuteEmail(Order order, string message) {
            ...
            _logger.LogInformation("Scriban template created succeessfully. Template: {result}", result);
            ...
            _logger.LogInformation("Email StatusCode: {response}", response.StatusCode);
        }
    }
}